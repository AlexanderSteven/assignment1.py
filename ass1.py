...
data_list = []
songs_file = "Songlist.csv"

def main():
    read_file()  # reads Songlist.csv file
    print("Songs To Learn 1.0 - by Alexander Steven")
...
def list_song():
    ...
    for index, data in enumerate(data_list):
        if data[2] == "y":
            print(index, "* {:40} - {:30} ({})".format(data[0], data[1], data[2]))
            notlearned += 1
        else:
            print(index, "{:40} - {:30} ({})".format(data[0], data[1], data[2]))
            learned += 1
...
def quit_program():
    count_songs()
    print("{} songs saved to Songlist.csv".format(count_songs()))
